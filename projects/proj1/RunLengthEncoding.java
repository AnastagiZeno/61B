...
public class RunLengthEncoding implements Iterable {
    ...
    public RunLengthEncoding(int width, int height) {
        ...
        this.width = width;
        this.height = height;
        RunNode head = new RunNode();
        RunNode node = new RunNode(head, null, 0 ,0, 0, width*height);
        head.setNext(node);
        this.head = head;
    }
    ...
    public RunLengthEncoding(int width, int height, int[] red, int[] green,
        ...
        RunNode head = new RunNode();
        this.head = head;
        RunNode currentPrev = head;
        for (int i=0; i< red.length; i++) {
            RunNode node = new RunNode(currentPrev, null, red[i], green[i], blue[i], runLengths[i]);
    ...
    public PixImage toPixImage() {
        ...
        while (ri.hasNext()) {
            ...
            while (length > 0) {
                ...
                pi.setPixel(w, h, (short)f4[0], (short)f4[1], (short)f4[2]);
                length--;
                h++;
            }
        }
    ...
    private void __init(PixImage image) {
        RunNode head = new RunNode();
        this.head = head;
        ...
        RunNode current = head;
        for (int i=0; i<width; i++) {
            for (int j=0; j<height; j++) {
                int red = (int)image.getRed(i,j);
                int green = (int)image.getGreen(i,j);
                int blue = (int)image.getBlue(i,j);
                if (red == prevRed && green == prevGreen && blue == prevBlue) {
                    current.setRunLength(current.getRunLength() + 1);
                } else {
                    RunNode node = new RunNode(current, null, red, green, blue, 1);
    ...
    public void check() {
        ...
        while (ri.hasNext()) {
            int[] next = ri.next();
            totalCnt += next[3];
            if (red == next[0] && green == next[1] && blue == next[2]) {
...
class RunNode {
    ...
    private int red, green, blue, runLength;

    RunNode(){this.prev = null; this.next = null; this.red = -1; this.green = -1; this.blue = -1; this.runLength = 0;}

    RunNode(RunNode prev, RunNode next, int red, int green, int blue, int runLength) {
        this.prev = prev;
        this.next = next;
        this.red = red;
        this.green = green;
    ...
    public void set4(int red, int green, int blue, int runLength) {
        this.red = red;
        this.green = green;
        this.blue = blue;
        this.runLength = runLength;
